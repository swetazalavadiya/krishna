...
const createCart = async function (req, res) {
  try {
      if (Object.keys(req.body).length == 0) {return res.status(400).send({ status: false, message: "Please add product to the cart" });}
  
      let {productId, quantity, totalPrice, totalItems} = req.body;
      let {userId}= req.params
      if(req.body.userId){
          if(req.decodedToken.id!=req.body.userId){return res.status(403).send({status:false, message:"not authorized user"})}
      }
      if(userId){
          if (!isValidObjectId(userId)) {return res.status(400).send({ status: false, message: "userId is not valid id" });}
      }
      if(!quantity){quantity=1}
      if(isNaN(quantity)){return res.status(400).send({status:false, message:"please provide valid quantity"})}
      if (quantity <= 0) {return res.status(400).send({ status: false, message: "Please enter the quantity of the product atleast 1" });}
      quantity= Number(quantity)

      if(!productId){return res.status(400).send({status:false, message:"Please enter productId inside body"})}
      if (!isValidObjectId(productId)) {return res.status(400).send({ status: false, message: "Please enter valid productId" });}
      let findProduct= await productModel.findOne({_id:productId, isDeleted:false})
      if(!findProduct){return res.status(404).send({status:false, message: "this product does not exist."})}
      
      let carted= await cartModel.findOne({userId:userId})
      if(req.body.cartId){
          if(req.body.cartId!=carted._id){return res.status(400).send({status:false, message:"cartId is not correct"})}
      }
      if(carted){
...
const updateCart=async function(req,res){
   ...
   if(Object.keys(data).length==0){return res.status(400).send({status:false,msg:" body is empty,please provide valid body"})}

   if(!cartId){return res.status(400).send({status:false,msg:"please provide cart ID"})}
   if(!isValidObjectId(cartId)){return res.status(400).send({status:false,msg:"please provide valid cart ID"})}

   const cart=await cartModel.findById(cartId)
   if(!cart){return res.status(404).send({status:false,msg:"cart does not exist"})}
   if(cart.userId.toString()!==userId){return res.status(400).send({status:false,msg:"user invalid"})}

   if(!productId){return res.status(400).send({status:false,msg:"please provide product ID"})}
   ...
   if(!removeProduct && removeProduct!==0){return res.status(400).send({status:false,msg:"please provide remove product"})}
  if(!([1,0]).includes(removeProduct)){return res.status(400).send({status:false,msg:"please provide valid remove product"})}

  if(removeProduct==1){
    if(cart.items.length==0){return res.status(404).send({status:false,msg:"you can not update an empty cart"}) }
    ...
    else if(i==cart.items.length-1){return res.status(404).send({status:false,msg:"this product does not exist in this cart"})}
  }
    }
